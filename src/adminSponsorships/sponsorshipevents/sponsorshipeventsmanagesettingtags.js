...
import { getAllSponsorEvents_APIcall } from '../slice-sponsorship/slice-sponsorshipEvents/slice-getAllSponsorEvents'

const Sponsormanagesettingtagpopup = (props) => {
  ...
  const authStat_getAllActiveEventsData = useSelector((state) => state.getAllSponsorEventsData)
  const authStat_getActualSponsorEventPayload = useSelector((state) => state.getSponsorEventsDetailsData)
  ...
  const handleClose = () => {
    ...
    setSearchingResult()
    setSearchingResult1()
    setUpdatedTargetGroupsList(authStat_getSponsorEventDetails.sponsorEventDetailsData.sponsor_info.sponsor_details.targeted_groups)
    setUpdatedTargetInterestList(authStat_getSponsorEventDetails.sponsorEventDetailsData.sponsor_info.sponsor_details.additional_interest)
    setSelectedItemsFromSearchingResult([])
  }
  ...
  useEffect(() => {
    if (authStat_getSponsorEventDetails.data.responseCode == 200 && authStat_getSponsorEventDetails.data.responseCode != undefined) {

      console.log(authStat_getSponsorEventDetails.sponsorEventDetailsData)
      // setUpdatedTargetGroupsList(authStat_getSponsorEventDetails.sponsorEventDetailsData.sponsor_info.sponsor_details.targeted_groups)
      // setUpdatedTargetInterestList(authStat_getSponsorEventDetails.sponsorEventDetailsData.sponsor_info.sponsor_details.additional_interest)

      setUpdatedTargetGroupsList(authStat_getSponsorEventDetails.data.responseBody.active_events[0].sponsor_info.sponsor_details.targeted_groups)
      setUpdatedTargetInterestList(authStat_getSponsorEventDetails.data.responseBody.active_events[0].sponsor_info.sponsor_details.additional_interest)
      // alert((authStat_getSponsorEventDetails.data.responseBody.active_events[0].sponsor_info.sponsor_details.targeted_groups).length)
      // alert((authStat_getSponsorEventDetails.data.responseBody.active_events[0].sponsor_info.sponsor_details.additional_interest).length)

      // console.log(authStat_getSponsorEventDetails.sponsorEventDetailsData.sponsor_info.sponsor_details.additional_interest)
    }
    else {
      setUpdatedTargetGroupsList([])
      setUpdatedTargetInterestList([])
    }
  }, [authStat_getSponsorEventDetails.data])


  // useEffect(() => {
  //   if (authStat_getAllActualSponsorEventsData.data.responseCode && authStat_getAllActualSponsorEventsData.data.responseBody != undefined) {
  //     
  //     const responseData = authStat_getAllActualSponsorEventsData.data.responseBody
  //     setActualSponsorEventList(authStat_getAllActualSponsorEventsData.data.responseBody)
  //     setActualSponsorEventId(authStat_getAllActualSponsorEventsData.data.responseBody.event_id)
  //     console.log(responseData)
  //   }
  //   else {
  //     setActualSponsorEventList([])
  //   }
  // }, [authStat_getAllActualSponsorEventsData.data])
    ...
    <>
      <div className=''>
        ...
        <button className='btn_emptycolor d-flex flex-row justify-content-center align-items-center' style={{ backgroundColor: "white" }} onClick={() => handleOpenDialog()} >
          <span className='text_color text-center controlEventBtn'>Control Event Settings</span>
        </button>
        <Modal className='modalss_width' show={show} onHide={() => setShow(false)} backdrop="static" >

          <div className='pt-2 pb-2 px-3'>
            <span className='font_style_top'>Control Sponsored Event Settings</span>
            <div className="mt-2">
              <span className="10 tags_boldertext">Events showing up in these groups</span>
            </div>
...
export default Sponsormanagesettingtagpopup