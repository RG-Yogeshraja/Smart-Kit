...
import { reset_Mobile_APICall } from './resetpasswordmobileslice'
import Loaders from "../../enableloader"
const ResetMobileSuccess = () => {
  ...
  console.log(res)
  const authStat = useSelector((state) => state.resetDataforMobile)
  ...
  useEffect(() => {
    ...
    if (authStat != undefined && authStat != "") {
      ...
      const { resetpassword_mobile } = authStat
      console.log("data", resetpassword_mobile)
      const { responseBody } = resetpassword_mobile
      console.log("login responseBody", responseBody)
      console.log("auth respomse success")
      if (authStat.resetpassword_mobile.responseCode === 200) {
        setstart(false)
        navigate("/Success")

      } else if (authStat.resetpassword_mobile.responseCode === 201) {
        setstart(false)
        seterror('invalid')
  ...
  const onSubmit = (data) => {
    ...
    dispatch(reset_Mobile_APICall(payload))
          ...
          <div className="card cardbar p-3 d-flex flex-column w-100 mx-2">
            ...
            <form onSubmit={handleSubmit(onSubmit)} >
              ...
              <div className="form-group mb-0">
                ...
                <div className={`d-flex flexs  ${errors.confirmPassword ? 'is-invalid' : ''
                  }`}   >
                  <input className={`forms  ${errors.confirmPassword ? '' : ''
                    }`}
...
export default connect(mapStateToProps, {})(ResetMobileSuccess)